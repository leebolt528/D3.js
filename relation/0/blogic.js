...
$.extend({
    ...
    qunee: function(id, data,object1) {
                ...
                function(image, i) {
                   ...
                   /*  nodeClick = null;
                    edges_path.style("", function(d) {
                        if (d.source.key !== d.target.key) {
                            if (d.status == "fast") {
                                ...
                                d3.select(this).style("stroke", object.lineColor2);
                            } else if (d.status == "slow") {
                                d3.select(this).attr("marker-end", "url(#resolved2)");
                                d3.select(this).style("stroke", object.lineColor1);
                            } else {
                                d3.select(this).attr("marker-end", "url(#resolved)");
                                d3.select(this).style("stroke", object.lineColor);
                            }
                        } else {
                            if (d.status == "fast") {
                    ...
                    edges_text.style("", function(d, i) {
                        if (d.status == "fast") {
                            d3.select(this).style("fill", object.lineColor2);
                        } else if (d.status == "slow") {
                            d3.select(this).style("fill", object.lineColor1);
                        } else {
                            d3.select(this).style("fill", object.lineColor);
                        }
                    }); */
                }
            )
        ...
        var edgeInfo = function(edge) {
                return `<div>
                        <div class="moveDrill" style="height:20px;cursor:move"></div>
                        <ul class="nav nav-tabs">
                            <li class="active"><a href="#home1" data-toggle="tab">Home</a></li>
                            <li role="presentation"><a href="#profile1" data-toggle="tab">Profile</a></li>
                            <li role="presentation"><a href="#message1" data-toggle="tab">Messages</a></li>
                        </ul>
                        <div class="tab-content">
                            <div class="tab-pane active" id="home1">home1+" "+${edge.relation}</div>
                            <div class="tab-pane" id="profile1">profile1+" "+${edge.relation}</div>
                            <div class="tab-pane" id="message1">message1+" "+${edge.relation}</div>
                        </div>
                    </div>
                    `;
            }
        ...
        var edges_path = container.append("g").selectAll(".edgepath")
            ...
            .attr({
                ...
                "stroke": function(d, i) {
                    if (d.status == "fast") {
                        return object.lineColor2;
                    } else if (d.status == "slow") {
                        return object.lineColor1;
                    } else {
        ...
        var nodeInfo = function(node) {
                return `<div>
                        ...
                        <ul class="nav nav-tabs">
                            ...
                            <li role="presentation"><a href="#message" data-toggle="tab">Messages</a></li>
                        </ul>
                        <div class="tab-content">
                            <div class="tab-pane active" id="home">home+" "+${node.name}</div>
                            <div class="tab-pane" id="profile">profile+" "+${node.name}</div>
                            <div class="tab-pane" id="message">message+" "+${node.name}</div>
                        </div>
                    </div>
                    `;
            }
